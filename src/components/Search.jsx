...
function Search({searchTerm,handleSearchCountries} ) {
  ...
  return (
      ...
      <FontAwesomeIcon className='absolute pl-5 top-1/2 -translate-y-1/2 border-none'  icon={faMagnifyingGlass} size="lg" style={{color: "#ffffff",}} />
          <input  value={searchTerm} onChange={(event)=>handleSearchCountries(event.target.value)} className='  pl-14   w-full placeholder:text-left  border-none rounded-md bg-dark-blue py-3 ' type="text" placeholder='Search for a country...' />
      </div>
      </>
